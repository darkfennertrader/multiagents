...
PDF_DIRECTORY = Path(__file__).parent.parent / "pdfs"


class MarkdownToPDFInput(BaseModel):
    markdown_text: str = Field(
        "Markdown text to convert to PDF, provided in valid markdown format"
    )
...
def markdown_to_pdf_file(markdown_text: str) -> str:
    ...
    pdf_path = PDF_DIRECTORY / f"{unique_id}.pdf"
    ...
    pdfkit.from_string(html_text, str(pdf_path), options=options)
