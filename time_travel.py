...
if __name__ == "__main__":
    ...
    while ai_bot.app.get_state(thread).next:  # type: ignore
        ...
        if _input == "y":
            ...
            current_values = ai_bot.app.get_state(thread)  # type: ignore
            print(current_values.values["messages"][-1].tool_calls)
            # Let's update these tool calls:
            ...
            # update the state with modification
            ai_bot.app.update_state(thread, current_values.values)  # type: ignore
            print("New modified state:")
            print(ai_bot.app.get_state(thread))  # type: ignore
            print("\nnext state: ", ai_bot.app.get_state(thread).next)  # type: ignore
            print("-" * 50)
            break
        else:
            print("aborting")
    ...
    for event in ai_bot.app.stream(None, thread):  # type: ignore
        for v in event.values():
            print(v)
